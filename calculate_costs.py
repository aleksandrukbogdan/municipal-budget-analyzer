...
# --- КОНФИГУРАЦИЯ КАТЕГОРИЙ (intstruction строки 196-260) ---
CAT_KEYWORDS = {
    # Группа «Объекты образования» (строки 197-207)
    "Объекты образования": [
        "школа", "сош", "лицей", "гимназия", 
        "детский сад", "детского сада",
        "лагерь", "загородный оздоровительный",
        "дошкольное", "дошкольного",
        "автогородок",
        "образовательн", "образования",
        "молодежь", "молодёжь"
    ],
    # Группа «Объекты здравоохранения, культуры, спорта» (строки 209-225)
    "Объекты здравоохранения, культуры, спорта": [
        "клуб",
        "культурный центр", "культурного центра",
        "досуговый центр", "досугового центра",
        "физкультурно-оздоровительн", "физкультур",
        "фок",
        "спортивный комплекс", "спортивного комплекса",
        "спортивн",
        "бассейн",
        "стадион",
        "спортивная площадка", "спортивной площадки",
        "ледовый корт", "ледового корта", "ледов",
        "лыжероллер",
        "выставочный зал", "выставочного зала",
        "библиотек",
        "больниц", "больничн",
        "здравоохранен"
    ],
    # Группа «Транспортная инфраструктура» (строки 227-235)
    "Транспортная инфраструктура": [
        "автомобильная дорога", "автомобильной дороги",
        "дорог",  # дорога, дорожн
        "улично-дорожн",
        "тротуар",
        "мост",
        "путепровод",
        "остановочный комплекс", "остановочного комплекса", "остановочн",
        "дорожное движение", "дорожного движения"
    ],
    # Группа «Благоустройство и ЖКХ» (строки 237-259)
    "Благоустройство и ЖКХ": [
        ...
        "газопровод",
        "благоустройств",
        "городская среда", "городской среды",
        "свалк",  # свалка
        "контейнерн",  # контейнерная площадка
        "озеленен",
        "кладбищ",
        "пожарн",
        "гидротехн",  # гидротехнические сооружения
        "колумбарий",
        "освещен",
        "энергосбережен"
    ]
...
def classify_category(rz_code, text):
    ...
    if rz_code:
        ...
        # 4. ЖКХ (раздел 05)
        if section == "05":
            # 0501 = Жилищное хозяйство - ИСКЛЮЧАЕМ (жилой фонд, МКД и т.д.)
            if subsection == "0501":
                return None  # Не включаем в отчёт
            # 0502 = Коммунальное хозяйство → ЖКХ
            # 0503 = Благоустройство → ЖКХ
            if subsection in ["0502", "0503"]:
...
class CostCalculator:
    ...
    def load_filtered_articles(self, articles, years=None):
        ...
        """
        if years:
            self.all_years = set(int(y) for y in years)
        
        skipped_0501 = 0
        for article in articles:
            # Определяем годы из данных (конвертируем в int)
            if 'sums' in article:
                self.all_years.update(int(y) for y in article['sums'].keys())
            
            # Категория определяется по коду РзПр (раздел/подраздел)
            category = classify_category(article.get('rz'), article.get('name', ''))
            
            # Пропускаем статьи с category=None (например, 0501 - Жилищное хозяйство)
            if category is None:
                skipped_0501 += 1
                continue
        ...
        if skipped_0501 > 0:
            print(f"  (пропущено {skipped_0501} статей из раздела 0501 - Жилищное хозяйство)")
    ...
    def calculate_totals(self):
        """Рассчитывает итоговые суммы по МО и категориям (в тыс. руб.)."""
        if not self.results:
            return {}
        ...
        # Конвертируем в тысячи рублей во всех агрегациях
        totals = {
            'by_mo': {k: {year: round(val/1000, 2) for year, val in v.items()} 
                      for k, v in df.groupby('МО')[year_cols].sum().to_dict().items()},
            'by_category': {k: {year: round(val/1000, 2) for year, val in v.items()} 
                            for k, v in df.groupby('Категория')[year_cols].sum().to_dict().items()},
            'by_mo_category': [{**row, **{y: round(row[y]/1000, 2) for y in year_cols}} 
                              for row in df.groupby(['МО', 'Категория'])[year_cols].sum().reset_index().to_dict('records')],
            'grand_total': {k: round(v/1000, 2) for k, v in df[year_cols].sum().to_dict().items()}
        }
    ...
    def _clean_for_excel(text):
        """Очищает текст от недопустимых символов Excel."""
        if text is None:
            return ""
        if not isinstance(text, str):
            return text
        # Удаляем управляющие символы (кроме tab, newline)
        import re
    ...
    def save_to_excel(self, output_path="output/extracted_budget_data.xlsx"):
        ...
        # Очищаем строковые колонки от недопустимых символов
        for col in df.select_dtypes(include=['object']).columns:
            df[col] = df[col].apply(self._clean_for_excel)
        ...
        for y in all_year_cols:
            if y not in df.columns:
                ...
                # Конвертируем в тысячи рублей и округляем до 2 знаков
                df[y] = (df[y].fillna(0.0) / 1000.0).round(2)
